...
#define IN4 8 //IN4 no pino 7 controla o outro lado da ponte B
...
void ativarAlarme(){
            tone(buzzerPin,1000); // buzzer é acionado
            digitalWrite(9, HIGH); // LED é aceso
            delay(1000); // tempo de espera
            noTone(buzzerPin); // buzzer é desligado
            digitalWrite(9, LOW); // LED é apagado
            delay(1000);
}

void procurarObstaculo(){
...
void loop() { // laço de repetição (repetição, até segunda ordem)
    if(cont < 50){ // quando atingir a contagem, o robo irá fazer a varredura
        ...
        if((sensor1+sensor2+sensor3) > 500){ //taxa de leitura dos sensores infravermelhos maiores que 500 (dentro da linha)
            ...
            cont++; //incrementa contagem que ativa a verificação de obstáculo
        }
         sensor1 = analogRead(0); //esquerda
         sensor3 = analogRead(2); //direita
        ...
        //realizar varredura na frente
        procurarObstaculo();
        for(pos = 90; pos <= 180; pos++){ // uso do servo motor, fazendo o ultrassonico girar
            myservo.write(pos);
            delay(10);
        }
        //realizar varredura ma esquerda
        procurarObstaculo();
        for(pos = 180; pos >= 90; pos--){ //retorno a posição inicial
            myservo.write(pos);
            delay(10);
        }
        delay(1000);
        for(pos = 90; pos >= 0; pos--){ //gir servo motor pra direita
            myservo.write(pos);
            delay(10);
        }
        //realizar varredura ma direita
        procurarObstaculo();
